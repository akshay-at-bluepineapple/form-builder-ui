                                    ...
                                    {row.columns.map(col => {
                                        ...
                                        return (<div key={col.id} className={`${colWidth} relative group rounded pt-6 px-2`}>
                                            <DndContext onDragEnd={e => {
                                                                    ...
                                                                    rows: section.rows.map(r =>
                                                                        r.id !== row.id ? r : {
                                                                            ...
                                                                            columns: r.columns.map(c =>
                                                                                c.id === col.id ? {
                                                                                    ...c,
                                                                                    fields: arrayMove(c.fields.config, from, to)
                                                                                } : c
                                                                            )
                                                                        }
                                                                    )
                                                ...
                                                <SortableContext items={col.fields?.map(f => f.config?.id)} strategy={verticalListSortingStrategy}>
                                                    <button
                                        ...
                                        {row.columns.map(col => (
                                            <div key={col.id} className="w-1/3 space-y-3">
                                                {col.fields.map(field => {
                                                    ...
                                                    return (
                                                        <div key={field.id}>
                                                            <label className="block font-medium">{config.label}</label>
                                                            {fieldType === 'text' && <input placeholder={config.placeholder} className="border p-1 rounded w-full" />}
                                                            {fieldType === 'label' && <div>{config.label}</div>}
                                                            {fieldType === 'checkbox' && <input type="checkbox" />}
                                                            {fieldType === 'date' && <input type="date" className="border p-1 rounded" />}
                                                        </div>)
                                                })}
                                            </div>
                                        ))}
